...
While it's possible to just query the oracle for each value one at a time, it's really not cost efficient.  Perhaps there's still something I can do with the raw bytes returned by the oracle.
