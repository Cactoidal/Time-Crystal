...
The first has to do with secret information.  Here, the player would provide the seed and iv as normal, and in addition, they will provide some kind of secret nonce or passphrase (they will have registered an AES key with the DON beforehand).  The oracle could generate secret randomness, for example, then append this to the player's secret nonce, hash the appended string, and commit the hash on-chain.
