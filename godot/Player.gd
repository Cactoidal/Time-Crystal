...
var menu_open = false
var not_options = false
var options_menu = load("res://OptionsMenu.tscn")
var menu
...
func _physics_process(delta):
	...
	if global_transform.origin.y < -30:
		get_parent().get_parent().fade("teleport")
	
	if Input.is_action_just_pressed("menu"):
		if menu_open == false:
			menu_open = true
			var new_menu = options_menu.instance()
			add_child(new_menu)
			new_menu.player = self
			menu = new_menu
			Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
		else:
			if not_options == false:
				menu_open = false
				menu.queue_free()
				Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
